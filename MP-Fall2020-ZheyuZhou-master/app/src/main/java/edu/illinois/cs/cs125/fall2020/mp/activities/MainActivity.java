...
public final class MainActivity extends AppCompatActivity
  ...
  protected void onCreate(final Bundle unused) {
    ...
    Log.i("Startup", "onCreate in CourseableApplication");
    Log.v("TestingTag", "Here");
  ...
  public boolean onQueryTextChange(final String query) {
    List<Summary> s = Summary.filter(courses, query);
    listAdapter.edit().replaceAll(s).commit();
  ...
  public void onCourseClicked(final Summary course) {
    Log.d(TAG, "Clicked on " + course.getNumber());
    Intent startCourseActivity = new Intent(this, CourseActivity.class);
    try {
      startCourseActivity.putExtra("COURSE", HELP.writeValueAsString(course));
